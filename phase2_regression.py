...
n, p = X_train.shape   # n=örnekler, p=özellik sayısı

# Bias (intercept) ekle → [1 | X]
...
def neg_log_likelihood(theta):
    beta  = theta[:-1]                     # (p+1,)
    log_s = theta[-1]                      # ln(σ)
    sigma = np.exp(log_s)                  # σ > 0 garantisi
...
beta_mle   = result.x[:-1]        # β tahminleri
sigma_mle  = np.exp(result.x[-1]) # σ tahmini
...
X_val_aug  = np.hstack([np.ones((len(X_val_s),  1)), X_val_s])
X_test_aug = np.hstack([np.ones((len(X_test_s), 1)), X_test_s])
...
for col, (title, resid, color) in enumerate(titles_row1):
    ...
    elif col == 1:  # Kalıntı zaman serisi
